...
import { fetchAPIAsync, notificationSwal, eliminarSwal, API_URL_NEGOCIO, API_HOST, API_URL_TIPO_TICKETS } from 'common/common';
...
const columns = [
  ...
  { id: 'tipo_id', label: 'NOMBRE', minWidth: 170, key: 1 },
...
export default function NegocioTipoTicketsPage() {
  const { id } = useParams();
  const [negocio, setNegocio] = React.useState({});
  const [tickets, setTickets] = React.useState([]);
  const [negocioTickets, setNegocioTickets] = React.useState([]);
  ...
  useEffect(() => {
    SearchFilter(page);
    fetch(API_URL_TIPO_TICKETS + 'all')
      .then((response) => response.json())
      .then((data) => setTickets(data))
      .catch((error) => console.error('Error en la obtencion de tickets:', error));
  }, []);
  ...
  async function SearchFilter(numPage) {
    ...
    try {
      const result = await fetchAPIAsync(API_URL_NEGOCIO + 'tickets', filter, 'GET');
      fetch(API_URL_NEGOCIO + `ticketsdata/${id}`)
        .then((response) => response.json())
        .then((data) => {
          setNegocio(data);
          setNegocioTickets(data?.tipotickets);
        })
  ...
  async function SaveItem() {
    ...
    try {
      await axios.post(API_URL_NEGOCIO + 'ticketsbloque', itemSave);
  ...
  async function handleEliminar(row) {
    ...
    eliminarSwal(elementoId, API_URL_NEGOCIO + 'tickets', SearchFilter);
  }
  ...
  function ValidacionItemSave(itemSave) {
    ...
    if (!itemSave || Object.keys(itemSave).length === 0) {
      ...
      if (!itemSave.tipos_id || Object.keys(itemSave.tipos_id).length === 0) {
        msgResponse += '* Debe seleccionar un Ticket.<br>';
        response = false;
      }
    }
        ...
        <div className="row content">
          <div className="col-sm-9">
            <form className="form row mb-4">
              ...
              <div className="col-sm-6">
                <label htmlFor="form_tipo_id">Ticket:</label>
                <Autocomplete
                  size="small"
                  options={negocioTickets}
                  getOptionLabel={(option) => option.name}
                  onChange={(event, newValue) => {
                    setSearchFilter({ ...searchFilter, form_tipo_id: newValue ? newValue.id : '' });
                  }}
                      ...
                      {columns.map((column, index) => {
                        return (
                          <TableCell key={index} align={column.align}>
                            {column.id === 'code' ? (
                              tickets.find((prov) => prov.id === row.tipo_id)?.code
                            ) : column.id === 'tipo_id' ? (
                              tickets.find((prov) => prov.id === row.tipo_id)?.name
                            ) : column.id === 'image' ? (
                              <div>
                                {tickets.find((prov) => prov.id === row.tipo_id)?.image ? (
                                  <img
                                    src={API_HOST + tickets.find((prov) => prov.id === row.tipo_id)?.image}
                                    alt={tickets.find((prov) => prov.id === row.tipo_id)?.image}
                                    style={{
                                      maxHeight: '50px',
                                      height: '100%',
                                      objectFit: 'cover'
                              ...
                              `PEN ${Number(tickets.find((prov) => prov.id === row.tipo_id)?.price).toFixed(2)}`
                            ) : (
                              row[column.id]
                            )}
                          </TableCell>
        ...
        <div className="modal-dialog modal-lg modal-dialog-centered d-flex justify-content-center align-items-center">
          <div className="modal-content">
            <div className="modal-header">
              <h3 className="modal-title">Agregar Tickets</h3>
                ...
                <Table size="small">
                  <TableHead>
                    <TableRow>
                        ...
                        <FormControlLabel
                          control={
                            <Checkbox
                              indeterminate={
                                selected.length > 0 &&
                                selected.length < tickets.filter((obj2) => !filteredRows.some((obj1) => obj1.tipo_id === obj2.id))?.length
                              }
                              checked={
                                selected?.length ===
                                tickets.filter((obj2) => !filteredRows.some((obj1) => obj1.tipo_id === obj2.id))?.length
                              }
                              onChange={() => {
                                if (
                                  selected?.length ===
                                  ...
                                  setSelected(
                                    tickets.filter((obj2) => !filteredRows.some((obj1) => obj1.tipo_id === obj2.id)).map((row) => row.id)
                                  );
                                }
                              }}
                            />
                      ...
                      {columnsTickets.map((column, index) => (
                        <TableCell key={index} align={column.align} style={{ minWidth: column.minWidth }}>
                          {column.label}
                        </TableCell>
                      ))}
                  ...
                  <TableBody>
                    {tickets
                      .filter((obj2) => !filteredRows.some((obj1) => obj1.tipo_id === obj2.id))
                      .map((row) => (
                        <TableRow key={row.id}>
                          <TableCell padding="checkbox">
                          ...
                          {columnsTickets.map((column, index) => (
                            <TableCell key={index} align={column.align}>
                              {column.id === 'image' ? (
                                ...
                                `PEN ${Number(row.price).toFixed(2)}`
                              ) : (
                                row[column.id]
                              )}
                            </TableCell>
