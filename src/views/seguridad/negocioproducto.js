...
import { fetchAPIAsync, notificationSwal, eliminarSwal, API_URL_PRODUCTO, API_URL_NEGOCIO, API_HOST } from 'common/common';
...
export default function NegocioProductoPage() {
  const { id } = useParams();
  const [negocio, setNegocio] = React.useState({});
  const [productos, setProductos] = React.useState([]);
  const [negocioProductos, setNegocioProductos] = React.useState([]);
  ...
  async function SearchFilter(numPage) {
    ...
    try {
      const result = await fetchAPIAsync(API_URL_NEGOCIO + 'producto', filter, 'GET');
      fetch(API_URL_NEGOCIO + `productodata/${id}`)
        .then((response) => response.json())
        .then((data) => {
          setNegocio(data);
          setNegocioProductos(data?.productos);
        })
  ...
  async function SaveItem() {
    ...
    try {
      await axios.post(API_URL_NEGOCIO + 'productobloque', itemSave);
  ...
  async function handleEliminar(row) {
    ...
    eliminarSwal(elementoId, API_URL_NEGOCIO + 'producto', SearchFilter);
  }
    ...
    <div>
      <div className="form">
        ...
        <Button onClick={handleOpenModal1} className="btn btn-info mb-3">
          Agregar Productos a {negocio?.name}
        </Button>
                      ...
                      {columns.map((column, index) => {
                        return (
                          <TableCell key={index} align={column.align}>
                            {column.id === 'code' ? (
                              productos.find((prov) => prov.id === row.producto_id)?.code
                            ) : column.id === 'producto_id' ? (
                              productos.find((prov) => prov.id === row.producto_id)?.name
                            ) : column.id === 'image' ? (
                              <div>
                                {productos.find((prov) => prov.id === row.producto_id)?.image ? (
                                  <img
                                    src={API_HOST + productos.find((prov) => prov.id === row.producto_id)?.image}
                                    alt={productos.find((prov) => prov.id === row.producto_id)?.image}
                                    style={{
                                      maxHeight: '50px',
                                      height: '100%',
                                      objectFit: 'cover'
                              ...
                              `${productos.find((prov) => prov.id === row.producto_id)?.moneda} ${Number(
                                productos.find((prov) => prov.id === row.producto_id)?.price
                              ).toFixed(2)}`
                            ) : (
                              row[column.id]
                            )}
