...
const columns = [
  ...
  { id: 'tipo_id', label: 'TIPO TICKET', minWidth: 170, key: 1 },
...
export default function UserTipoTicketPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    fetch(API_URL_TIPO_TICKETS + `?paginate=10000&negocio_id=${negocio_id}`)
      .then((response) => response.json())
      .then((data) => setTipos(data?.data))
      .catch((error) => console.error('Error en la obtencion de tipos:', error));
    SearchFilter(page);
  }, []);
  ...
  async function SearchFilter(numPage) {
    ...
    try {
      const result = await fetchAPIAsync(API_URL_USER_TIPO_TICKET, filter, 'GET');
  ...
  async function SaveItem() {
    ...
    try {
      await axios.post(API_URL_USER_TIPO_TICKET + 'bloque', itemSave);
  ...
  async function handleEliminar(row) {
    ...
    eliminarSwal(elementoId, API_URL_USER_TIPO_TICKET, SearchFilter);
  }
  ...
  function ValidacionItemSave(itemSave) {
    ...
    if (!itemSave || Object.keys(itemSave).length === 0) {
      ...
      if (!itemSave.tipos_id || Object.keys(itemSave.tipos_id).length === 0) {
        msgResponse += '* Debe seleccionar un Tipo.<br>';
        response = false;
      }
    }
        ...
        <div className="row content">
          <div className="col-sm-9">
            <form className="form row mb-4">
              ...
              <div className="col-sm-4">
                <label htmlFor="form_tipo_id">Tipo:</label>
                <Autocomplete
                  size="small"
                  options={tipos}
                ...
                {filteredRows.map((row, index) => {
                  return (
                    <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                      {columns.map((column, index) => {
                        const tipo = tipos.find((prov) => prov.id === row.tipo_id);
                        return (
                          <TableCell key={index} align={column.align}>
                            {column.id === 'code' ? (
                              ...
                              `PEN ${Number(tipo.price).toFixed(2)}`
                            ) : (
                              row[column.id]
                            )}
                          </TableCell>
        ...
        <div className="modal-dialog modal-lg modal-dialog-centered d-flex justify-content-center align-items-center">
          <div className="modal-content">
            <div className="modal-header">
              <h3 className="modal-title">Agregar tipos</h3>
                        ...
                        <FormControlLabel
                          control={
                            <Checkbox
                              indeterminate={
                                selected.length > 0 &&
                                selected.length < tipos.filter((obj2) => !filteredRows.some((obj1) => obj1.tipo_id === obj2.id)).length
                              }
                              checked={
                                selected.length === tipos.filter((obj2) => !filteredRows.some((obj1) => obj1.tipo_id === obj2.id)).length
                              }
                              onChange={() => {
                                if (
                                  ...
                                  setSelected(
                                    tipos.filter((obj2) => !filteredRows.some((obj1) => obj1.tipo_id === obj2.id)).map((row) => row.id)
                                  );
                                }
                              }}
                            />
