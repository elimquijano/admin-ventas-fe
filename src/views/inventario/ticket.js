...
import { API_HOST, API_URL_TIPO_TICKETS, fetchAPIAsync, notificationSwal, eliminarSwal, getSession, API_URL_NEGOCIO } from 'common/common';
...
const columns = [
  ...
  { id: 'negocio_id', label: 'EMPRESA', minWidth: 100, key: 7 }
];

export default function TipoTicketPage() {
  ...
  async function SearchFilter(numPage) {
    ...
    try {
      const result = await fetchAPIAsync(API_URL_TIPO_TICKETS, filter, 'GET');
  ...
  async function SaveItem() {
    ...
    try {
      const result = await fetchAPIAsync(API_URL_TIPO_TICKETS, itemSave, 'POST');
  ...
  async function updateItem() {
    ...
    const result = await fetchAPIAsync(API_URL_TIPO_TICKETS + `update/${elementoId}`, editedItem, 'POST');
  ...
  function handleEliminar(row) {
    ...
    eliminarSwal(elementoId, API_URL_TIPO_TICKETS, SearchFilter);
  }
    ...
    <div>
        ...
        <div className="row content">
          <div className="col-sm-9">
            <form className="form row mb-4">
              ...
              <div className="col-sm-4">
                <label htmlFor="form_code">Código Interno:</label>
                <input
                  type="text"
                  className="form-control"
                  name="form_code"
                      ...
                      {columns.map((column, index) => (
                        <TableCell key={index} align={column.align}>
                          {column.id === 'image' ? (
                            ...
                            `${row.moneda || 'PEN'} ${Number(row.price).toFixed(2)}`
                          ) : column.id === 'negocio_id' ? (
                            negocios.find((n) => n.id == row[column.id])?.name
                          ) : (
                            row[column.id]
                          )}
                        </TableCell>
        ...
        <div className="modal-dialog modal-dialog-centered d-flex justify-content-center align-items-center">
          <div className="modal-content">
            <div className="modal-header">
              <h2 className="modal-title">Agregar tipo de ticket</h2>
      ...
      >
        <div className="modal-dialog modal-dialog-centered d-flex justify-content-center align-items-center">
          <div className="modal-content">
            <div className="modal-header">
              <h2 className="modal-title">Editar Tipo de ticket</h2>
