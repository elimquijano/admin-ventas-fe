...
import { redirectToRelativePage, deleteSession } from 'common/common';
...
const ProfileSection = ({ user }) => {
  ...
  const handleLogout = async () => {
    ...
    deleteSession('LATEST_NUMBER');
    deleteSession('PRIVILEGIOS');
    deleteSession('NEG_ID');
                ...
                <MainCard border={false} elevation={16} content={false} boxShadow shadow={theme.shadows[16]}>
                  <Box sx={{ p: 2 }}>
                    <Stack>
                      ...
                      <Typography variant="subtitle2">{user?.negocio === null ? '' : user?.negocio?.name}</Typography>
                    </Stack>
                    <OutlinedInput
